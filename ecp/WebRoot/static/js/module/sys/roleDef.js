...
function empower(id){
	$("#rol_col").tree({
    	 url:'role/rolePurvDefTree.do?typeId='+id+"&tt="+Math.floor(Math.random()*20)+1,
    	 method:'get',
    	 animate:true,
    	 checkbox:true
     });
	typeId = id;
	if(id=='1'){
		$("#rolePower").window("open").dialog("setTitel","设置默认管理员权限");
	}else if(id=='2'){
		$("#rolePower").window("open").dialog("setTitel","设置默认角色权限");		
	}
...
function passPower(){
	...
	$.post('role/addRolePurvDef.do', 'colIds='+s+'&typeId='+typeId, function(result) {
