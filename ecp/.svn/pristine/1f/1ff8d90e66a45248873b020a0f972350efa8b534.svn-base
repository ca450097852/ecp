...
function f_timeout(data) {
	if (data!=null&&data.rows!=null&&data.rows.statusCode != null) {
		if (data.rows.statusCode == 301) { 
			$.alert('用户会话已超时，请重新登录！','提示',function(){
				top.window.parent.location = 'login/ns_login.do';				
			});			
		}else if(data.rows.statusCode == 302){
			alert('用户会话已超时，请重新登录！');
			top.window.parent.location = '/ecp';
		}
	}
}

function checkFieldValue(fieldId,msg){
	
	var $field = $("#"+fieldId);
	var fieldValue='';
	if($field.attr("class").indexOf("combo-f")>0){
		var fieldName = $field.attr("textboxname");
		fieldValue = $("input[name='"+fieldName+"']").val();
	}else{
		fieldValue = $field.val();
	}
	
	if(fieldValue==""||fieldValue=='undefinded'){
		$field.tips({side:3, msg:msg, bg:'#AE81FF',time:2});
		$field.focus();
		return false;
	}
	return true;
...
function setFormFieldValue(data,form){
...
function resetFormFieldValue(target){
...
/*$.extend($.fn.validatebox.defaults.rules, {
   ...
   //第3个参数：服务器端验证返回的出错提示
   //<input type="text" name="user_name" id="username" class="easyui-validatebox" required="true"
   //validType="uniqueName['checkInstru/findCheckInstruName','name','该用户名已经存在']"/>
   uniqueName: {  
       validator: function(value, param) {
           var postdata = {};  
