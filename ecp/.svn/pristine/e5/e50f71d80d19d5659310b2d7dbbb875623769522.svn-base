...
public class RecommendGoodsServiceImpl implements RecommendGoodsService {
	...
	public JSONObject getRecommend(Integer memberId) {
		...
		SearchPageUtil page2 = new SearchPageUtil();
		page2.getParams().put("memberIdBuy", memberId);
		page2.getParams().put("orderState", "4");
		page2.getParams().put("recommend", "1");
		page2.setRows(100);		
		
		int orderCount = orderDao.getCountWithParam(page2);
		
		double recommendMoney = 0;
		page2.setRows(10000);		
		List<TbOrder> list = orderDao.findPageListWithParam(page2);
		for (TbOrder tbOrder : list) {
			recommendMoney+=Double.valueOf(tbOrder.getOrderAmount());
		}
		
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("recommendGoods", rows.size());
		jsonObject.put("recommendOrders", orderCount);
		jsonObject.put("recommendMoney", recommendMoney);
