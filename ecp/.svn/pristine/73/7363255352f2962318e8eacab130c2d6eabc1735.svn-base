...
function UserNameIsNull(){
    ...
    var reg_username = /^[a-zA-Z][a-zA-Z0-9|_]{3,19}$/;//由数字、26个英文字母或者下划线组成的字符串
...
function checkCompanyname(){
		...
		$.ajax({
		   type:"post", 
		   url:"/portal/member!checkCompanyname.action?cust_name="+vtext+"&ajaxRequestRandom="+Math.random(), 
		   datatype:"json", 
		   async:false, 
		   success:function (data) {
			if (data == "1") {
