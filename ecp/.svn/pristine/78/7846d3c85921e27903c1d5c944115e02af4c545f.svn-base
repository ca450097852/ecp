...
function createOrder(){
	...
	var params = {"goodsId":<%=goodsId%>,"goodsCount":goodsCount,"buyRemark":buy_remark,"addrId":addrId,"shipFree":shipFree};
	if(!addrId){
		$.toast("请选择收货地址!");
		return;
	}
	$.ajax({
		   type: "POST",
		   data: params,
		   url: "order/createOrderOneGoods.mobile",
		   dataType:"text",
		   async:false,
		...
		$.ajax({
		   ...
		   success: function(data){
				if(data){
						...
						var totalMoney=(parseFloat(row.saledPrice*100)*goodsCount)/100;  															
						$("#totalMoney").html(totalMoney);	
